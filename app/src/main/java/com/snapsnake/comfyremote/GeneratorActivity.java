...
public class GeneratorActivity extends Activity {
    ...
    private void buildUi() {
        ...
        addTopButton(topRow, "Params", this::showParams);
        ...
        paramsPane = new ScrollView(this);
        paramsPane.setVerticalScrollBarEnabled(true);
        paramsPane.setScrollbarFadingEnabled(false);
        content = new LinearLayout(this);
        content.setOrientation(LinearLayout.VERTICAL);
        content.setPadding(dp(14), dp(14), dp(14), dp(88));
        paramsPane.addView(content, new ScrollView.LayoutParams(-1, -2));
        workspace.addView(paramsPane, new FrameLayout.LayoutParams(-1, -1));
        ...
        addToolButton(bottom, "Params", this::showParams);
    ...
    private void renderParams() {
        ...
        content.addView(muted("Create first. The node map is only for inspection."));
        if (workflow == null) {
            sourceCard();
            LinearLayout c = card();
            c.addView(header("No workflow imported"));
            c.addView(muted("Open Graph, load workflow, wait until nodes are visible, then press Import."));
    ...
    private void importedCard() {
        LinearLayout c = cardAccent(); content.addView(c, cardParams());
        c.addView(header("Workflow imported"));
        c.addView(muted(workflow.length() + " nodes loaded. Editable fields are grouped below."));
    }

    private void createCard() {
        LinearLayout c = cardAccent(); content.addView(c, cardParams());
        c.addView(header("Create"));
        c.addView(muted("All direct editable workflow fields. This is the main editing area."));
        List<String> ids = editableNodeIds();
        if (ids.isEmpty()) {
            c.addView(muted("No editable widget fields found. Re-import after the Graph workflow is fully loaded."));
            return;
        }
        for (String id : ids) addControlGroup(c, id);
    }
    ...
    private void generateCard() {
        ...
        addCardButton(r, "Apply", false, () -> { applyFields(); saveWorkflow(); toast("Applied"); renderParams(); });
    ...
    private void workflowMapCard() {
        ...
        LinearLayout c = card(); content.addView(c, cardParams());
        c.addView(header("Workflow map"));
        c.addView(muted("Secondary view. Tap a node only to inspect links or a specific technical node."));
        if (ids.isEmpty()) return;
        LinearLayout r = row(); c.addView(r);
        addCardButton(r, "First editable", false, () -> { applyFields(); selectedNodeId = firstEditableNodeId(ids); renderParams(); });
        addCardButton(r, "Next editable", true, () -> { applyFields(); selectedNodeId = nextEditableNodeId(ids, selectedNodeId); renderParams(); });
        LinearLayout row = null;
        for (int i = 0; i < ids.size(); i++) {
            if (i % 2 == 0) { row = row(); row.setPadding(0, dp(6), 0, 0); c.addView(row); }
            addNodeTile(row, ids.get(i));
        }
        if (ids.size() % 2 == 1 && row != null) row.addView(new View(this), weight(dp(86)));
    }
    ...
    private void addNodeTile(LinearLayout parent, String id) {
        ...
        b.setOnClickListener(v -> { applyFields(); saveWorkflow(); selectedNodeId = id; renderParams(); });
    ...
    private void selectedNodeCard() {
        ...
        c.addView(muted("Inspector. Main editable fields are already in Create above."));
        boolean added = false;
        for (String key : inputKeys(inputs)) {
            Object value = inputs.opt(key);
            if (!primitive(value)) continue;
    ...
    private boolean addLinkedInputs(LinearLayout c, JSONObject inputs) {
        ...
        for (String key : inputKeys(inputs)) {
            ...
            jump.setOnClickListener(v -> { applyFields(); saveWorkflow(); selectedNodeId = ref; renderParams(); });
    ...
    private void showParams() {
        paramsPane.setVisibility(View.VISIBLE);
        graph.setVisibility(View.GONE);
        output.setVisibility(View.GONE);
        renderParams();
        status.setText("Params ready. Tap this line to show/hide URL panel.");
        applyBars();
    }

    private void showGraph() {
        saveUrl();
        paramsPane.setVisibility(View.GONE);
        output.setVisibility(View.GONE);
    ...
    private void handleImportJson(String decoded) {
        try {
            ...
            saveWorkflow(); saveOptions(); selectedNodeId = null;
    ...
    private void applyJson() {
        try {
            ...
            fieldOptions = new JSONObject(); saveWorkflow(); saveOptions(); selectedNodeId = null;
            toast("Workflow loaded"); renderParams();
        } catch (JSONException e) { toast("Invalid JSON"); }
    }
    private void applyJsonText(String raw) {
        try { workflow = cleanWorkflow(new JSONObject(raw)); fieldOptions = new JSONObject(); saveWorkflow(); saveOptions(); selectedNodeId = null; toast("Workflow loaded"); renderParams(); }
    ...
    private void uploadImage(Uri uri, String node, String key) {
        ...
        new Thread(() -> {
            try {
                ...
                handler.post(() -> { busy(false, "Uploaded image: " + uploaded); renderParams(); });
        ...
        try {
            ...
            new Thread(() -> {
                try {
                    ...
                    handler.post(() -> { busy(false, "Queued. Waiting for output..."); updateGenerationUi(15, "Queued."); pollHistory(); });
    ...
    @Override public void onBackPressed() { if (output.getVisibility() == View.VISIBLE) { showParams(); return; } if (graph.getVisibility() == View.VISIBLE) { if (graph.canGoBack()) graph.goBack(); else showParams(); return; } super.onBackPressed(); }
