...
public class StudioActivity extends Activity {
    ...
    private void renderCreateScreen() {
        content.addView(text("Create", 26, Color.WHITE));
        content.addView(muted("One editable group at a time. No node hunting, no endless page."));
        importedCard();
        controlPickerCard();
        generateCard();
    }
    ...
    private void renderMapScreen() {
        ...
        content.addView(muted("Technical view only. Editing belongs to Create."));
        workflowMapCard();
        selectedNodeCard();
    }
    ...
    private void importedCard() {
        ...
        c.addView(muted(workflow.length() + " nodes loaded. Create shows editable groups; Map is separate."));
        LinearLayout r = row(); c.addView(r);
        addCardButton(r, "Re-import", false, this::importFromGraph);
        addCardButton(r, "Open Map", true, this::showMap);
    }

    private void controlPickerCard() {
        List<String> ids = editableNodeIds();
        LinearLayout c = cardAccent(); content.addView(c, cardParams());
        c.addView(header("Controls"));
        c.addView(muted("This is not important/unimportant sorting: it is a picker of nodes that actually have editable fields."));
        if (ids.isEmpty()) {
        ...
        LinearLayout r = row(); c.addView(r);
        addCardButton(r, "Pick", false, () -> showControlPicker(ids));
        addCardButton(r, "Next", true, () -> { applyFields(); saveWorkflow(); selectedControlNodeId = nextEditableNodeId(ids, selectedControlNodeId); renderStudio(); });
    ...
    private void showControlPicker(List<String> ids) {
        ...
        new AlertDialog.Builder(this).setTitle("Pick editable group").setItems(items, (d, which) -> {
    ...
    private String controlLabel(String id) {
        ...
        return "#" + id + "  " + prettify(cls) + "  ·  " + editableCount(id, inputs) + " fields";
    }
    ...
    private void workflowMapCard() {
        ...
        c.addView(muted("Map is separate from Create. Empty technical nodes no longer block the editing flow."));
    ...
    private void selectedNodeCard() {
        ...
        c.addView(muted("Inspector. Main editing is in Create; this panel is for technical checking."));
        boolean added = false;
        for (String key : inputKeys(inputs)) {
            Object value = inputs.opt(key);
            if (!primitive(value)) continue;
    ...
    private void handleImportJson(String decoded) {
        try {
            ...
            saveWorkflow(); saveOptions(); selectedControlNodeId = null; selectedMapNodeId = null;
    ...
    private void applyJson() {
        try {
            ...
            fieldOptions = new JSONObject(); saveWorkflow(); saveOptions(); selectedControlNodeId = null; selectedMapNodeId = null;
            toast("Workflow loaded"); renderStudio();
        } catch (JSONException e) { toast("Invalid JSON"); }
    }
    private void applyJsonText(String raw) {
        try { workflow = cleanWorkflow(new JSONObject(raw)); fieldOptions = new JSONObject(); saveWorkflow(); saveOptions(); selectedControlNodeId = null; selectedMapNodeId = null; toast("Workflow loaded"); renderStudio(); }
