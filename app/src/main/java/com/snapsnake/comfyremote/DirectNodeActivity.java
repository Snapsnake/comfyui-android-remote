...
public class DirectNodeActivity extends Activity {
    ...
    private void renderCreate() {
        ...
        content.addView(text("Create", 26, Color.WHITE));
        content.addView(muted(workflow.length() + " nodes. Choose a node directly; no categories and no Next loop."));
        nodePickerCard();
        selectedEditorCard();
        generateCard();
    }
    ...
    private void nodePickerCard() {
        LinearLayout c = card(); content.addView(c, cardParams());
        c.addView(header("Node"));
        c.addView(muted(selectedSummary(selectedNodeId)));
        LinearLayout r = row(); c.addView(r);
        addCardButton(r, "Choose node", true, this::showNodePicker);
        addCardButton(r, "All node tiles", false, this::showNodes);
    }
    ...
    private void selectedEditorCard() {
        ...
        if (!added && !linked) c.addView(muted("No local editable fields. Use linked inputs or choose another node."));
    }

    private void renderNodes() {
        content.addView(text("Nodes", 26, Color.WHITE));
        content.addView(muted("All workflow nodes as tiles. Tap a tile to edit that node in Create."));
        LinearLayout c = card(); content.addView(c, cardParams());
    ...
    private void addNodeTile(LinearLayout parent, String id) {
        ...
        int list = dropdownCount(id, inputs);
        ...
        Button b = button("#" + id + "\n" + shorten(prettify(cls), 24) + "\nedit " + edit + "   list " + list, color, 12, 14);
        b.setGravity(Gravity.LEFT | Gravity.CENTER_VERTICAL);
        b.setPadding(dp(10), 0, dp(8), 0);
        b.setOnClickListener(v -> { applyFields(); saveWorkflow(); selectedNodeId = id; showCreate(); });
    ...
    private void showNodePicker() {
        ...
        for (int i = 0; i < ids.size(); i++) items[i] = selectedSummary(ids.get(i));
        new AlertDialog.Builder(this).setTitle("Choose node").setItems(items, (d, which) -> {
            applyFields(); saveWorkflow(); selectedNodeId = ids.get(which); showCreate();
        }).show();
    }

    private String selectedSummary(String id) {
        if (id == null || workflow == null) return "No node selected";
        JSONObject node = workflow.optJSONObject(id);
        if (node == null) return "No node selected";
        JSONObject inputs = node.optJSONObject("inputs");
        return "#" + id + "  " + prettify(node.optString("class_type", "Node")) + "  ·  " + fieldPreview(inputs);
    }

    private String fieldPreview(JSONObject inputs) {
        List<String> keys = new ArrayList<>();
        for (String k : inputKeys(inputs)) if (inputs != null && primitive(inputs.opt(k))) keys.add(k);
        if (keys.isEmpty()) return "no local fields";
        String out = "";
        for (int i = 0; i < keys.size() && i < 4; i++) out += (i == 0 ? "" : ", ") + keys.get(i);
        if (keys.size() > 4) out += "…";
        return out;
    }
